...
g = Graph()
g.parse("actor5.ttl", format="turtle")
...
WHERE {
    ...
    BIND(URI(CONCAT("http://example.org/movies/", ?code)) AS ?movie)
    BIND(URI(CONCAT("http://example.org/actors/", ?actorId)) AS ?actor)
}
"""
...
output_graph.serialize(destination="output2.ttl", format="turtle")