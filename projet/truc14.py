...
WHERE {
    ...
    # Associer un seul identifiant unique pour chaque acteur
    {
        SELECT ?actorName ?actor (MIN(?actorId) AS ?uniqueActorId)
        WHERE {
            ?s <https://schema.org/actor> ?actorName ;
               <https://schema.org/identifier> ?actorId .
            BIND(URI(CONCAT("http://example.org/actors/", REPLACE(?actorName, " ", "_"))) AS ?actor)
        }
        GROUP BY ?actorName ?actor
    }
}
...
output_graph.serialize(destination="output14-b.ttl", format="turtle")